#!/usr/bin/env python3
# main.py - scripeterenotp OTP Spammer (FIXED)
...
def print_banner(tick=0):
    ...
    title = gradient_text("SCRIPETEREN OTP", tick, 0)
  ...
  ┌──────────────────────────────────────────────────────┐
  │  {color}API{reset}  : 122 │  {color}Version{reset}  : 1.0.0  │  {color}Dev{reset}  : SCRIPETEREN     │
  └──────────────────────────────────────────────────────┘{reset}
"""
    print(banner)
...
def show_execution_table(data, tick=0):
    ...
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  {Fore.GREEN}API Total :{Style.RESET_ALL} {Fore.YELLOW}{data.get('total_api', 122)}{' ' * (58 - 12 - len(str(data.get('total_api', 122))))}{Fore.CYAN}│{Style.RESET_ALL}")
...
def update_exec_data(name, status, detail=""):
    ...
    if status == "SUCCESS":
        ...
        exec_data['failed'] += 1
    exec_data['sent'] += 1
    total = exec_data.get('total_api', 122)
...
def run_with_ui(engine_func, target, threads=5):
    ...
    exec_data['total_api'] = len(TARGETS) if 'TARGETS' in globals() else 122
...
def main():
    try:
        ...
        if is_kali or is_termux:
            print(f"{Fore.GREEN}✓ Mode {('Kali' if is_kali else 'Termux')} terdeteksi{Style.RESET_ALL}")
